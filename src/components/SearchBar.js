    ...
    const handleInputChange = (e) => {
        ...
        if (value) {
            const filtered = behanceImages
                .filter((item) =>
                    item.UserName && item.UserName.toLowerCase().startsWith(value.toLowerCase())
                )
        ...
        <div className="search-area p-5 relative">
            <div className="search-area flex items-center">
                <div className="search-box w-full border-2 rounded-full bg-[#f9f9f9] overflow-hidden flex items-center justify-between">
                    <div className="input-box lg:w-full relative">
                        <input 
                            type="text" 
                        ...
                        {filteredSuggestions.length > 0 && (
                            <div className="absolute left-0 right-0 mt-2 w-full z-10">
                        ...
                        <div className='absolute left-6 bottom-1'>
                            <div className="search-icon text-2xl text-[#777]">
                                <IoSearchSharp />
                            </div>
                        </div>
                    </div>
                    <div className="tags-search bg-white px-4 py-3 border-l-2 ">
                        <ul className='flex items-center'>
                            ...
                            {
                                searchElements.map((tags, index) => (
                                    <li key={index} className='mx-1 font-medium text-sm py-1 px-3 hidden lg:block rounded-full hover:bg-[#eee]'><a href="">{tags.SearchItems}</a></li>
                                ))
                            }
                        </ul>
                    </div>
